...
function SortingProvider({ children }) {
    ...
    const bubbleSort = async () => {
        ...
        for (let i = 0; i < arr.length; i++) {
            for (let j = 0; j < arr.length - i - 1; j++) {
                ...
                if (arr[j] > arr[j + 1]) {
                    changeBar(j, { state: "swapping" });
                    changeBar(j + 1, { state: "swapping" });
                    await awaitTimeout(sortingState.delay / 2);
    ...
    const insertionSort = async () => {
        ...
        for (let i = 1; i < arr.length; i++) {
            ...
            changeBar(i, { value: current, state: "comparing" });
    ...
    const selectionSort = async () => {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            if (min !== i) {
                changeBar(i, { state: "swapping" });
                changeBar(min, { state: "swapping" });
                await awaitTimeout(sortingState.delay / 2);
    ...
    const startVisualizing = async () => {
        ...
        setSortingState((prev) => {
            const newArray = prev.array.map(item => ({ ...item, state: "sorted" }));
            return {
                ...prev,
                array: newArray,
                sorted: true,
